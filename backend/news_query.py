...
def real_time_query(time_range):
    ...
    url = f'https://www.alphavantage.co/query?function=NEWS_SENTIMENT&time_from={start_date}&limit=1000&apikey={api_key}'
    r = requests.get(url)
    data = r.json()
    ...
    for idx, label in enumerate(labels):
        if label not in events:
            events[label] = Event(label,summary=news_list[idx].summary, news_list=[])
